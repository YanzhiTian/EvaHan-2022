...
predict_path = './data/predict.txt'
result_path = './data/dev.txt'
def count_prf(contestant, answer):
    outcome = dict()
    with open(contestant, encoding='utf-8') as con:
        read_con = [i for i in con.read().split('\n') if i]

    with open(answer, encoding='utf-8') as ans:
...
def print_result():
    value = count_prf(predict_path, result_path)
    tool_line = "+" + "-" * 17 + '+' + "-" * 9 + '+' + '-' * 9 + '+' + '-' * 9 + '+'
    ...
    ouput = 'eval_outcome.txt'
    with open(ouput, 'a', encoding='utf-8') as fo:
        fo.write('\n' + task_line + '\n')
        fo.write(wsg_line + '\n' + pos_line +'\n')
